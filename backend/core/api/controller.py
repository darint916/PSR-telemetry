...
def post_battery():
    ...
    new_data_url = url_for('api.get_battery_data_by_id', id=id)
    return APIResponse.post_success(AddData(True, id, 'battery'), location=new_data_url).make()
...
def get_battery_data():
    data = get_data_all(csv_file_path + "battery.csv", battery_schema)
...
def get_battery_data_by_id(id):
    ...
    if time: data = get_data(csv_file_path + "battery.csv", battery_schema, time=time)
    else: data = get_data(csv_file_path + "battery.csv", battery_schema, id=id)
...
def post_temperature():
    ...
    if not id: return APIResponse.error("Failed to add data", 500).make()
    new_data_url = url_for('api.get_temperature_data_by_id', id=id)
    return APIResponse.post_success(AddData(True, id, 'temperature'), new_data_url).make()

@api.route("/telemetry/temperature/", methods=["GET"])
def get_temperature_data():
    data = get_data_all(csv_file_path + "temperature.csv", temperature_schema)
    if not data: return APIResponse.error("Failed to get data or not found", 500).make()
    return APIResponse.get_success(GetData(True, data)).make()
...
def get_temperature_data_by_id(id):
    time = request.args.get('time')
    if time: data = get_data(csv_file_path + "temperature.csv", temperature_schema, time=time)
    else: data = get_data(csv_file_path + "temperature.csv", temperature_schema, id=id)
    if not data: return APIResponse.error("Data not found", 404).make()
    return APIResponse.get_success(GetData(True, data)).make()
...
def post_speed():
    ...
    if not id: return APIResponse.error("Failed to add data", 500).make()
    new_data_url = url_for('api.get_speed_data_by_id', id=id)
    return APIResponse.post_success(AddData(True, id, 'speed'), new_data_url).make()

@api.route("/telemetry/speed/", methods=["GET"])
def get_speed_data():
    data = get_data_all(csv_file_path + "speed.csv", speed_schema)
    if not data: return APIResponse.error("Failed to get data or not found", 500).make()
    return APIResponse.get_success(GetData(True, data)).make()
...
def get_speed_data_by_id(id):
    time = request.args.get('time')
    if time: data = get_data(csv_file_path + "speed.csv", speed_schema, time=time)
    else: data = get_data(csv_file_path + "speed.csv", speed_schema, id=id)
    if not data: return APIResponse.error("Data not found", 404).make()
    return APIResponse.get_success(GetData(True, data)).make()
...
def post_engine():
    ...
    if not id: return APIResponse.error("Failed to add data", 500).make()
    new_data_url = url_for('api.get_engine_data_by_id', id=id)
    return APIResponse.post_success(AddData(True, id, 'engine'), new_data_url).make()

@api.route("/telemetry/engine/", methods=["GET"])
def get_engine_data():
    data = get_data_all(csv_file_path + "engine.csv", engine_schema)
    if not data: return APIResponse.error("Failed to get data or not found", 500).make()
    return APIResponse.get_success(GetData(True, data)).make()
...
def get_engine_data_by_id(id):
    time = request.args.get('time')
    if time: data = get_data(csv_file_path + "engine.csv", engine_schema, time=time)
    else: data = get_data(csv_file_path + "engine.csv", engine_schema, id=id)
    if not data: return APIResponse.error("Data not found", 404).make()
    return APIResponse.get_success(GetData(True, data)).make()
...
def post_solar():
    ...
    if not id: return APIResponse.error("Failed to add data", 500).make()
    new_data_url = url_for('api.get_solar_data_by_id', id=id)
    return APIResponse.post_success(AddData(True, id, 'solar'), new_data_url).make()

@api.route("/telemetry/solar/", methods=["GET"])
def get_solar_data():
    data = get_data_all(csv_file_path + "solar.csv", solar_schema)
    if not data: return APIResponse.error("Failed to get data or not found", 500).make()
    return APIResponse.get_success(GetData(True, data)).make()
...
def get_solar_data_by_id(id):
    time = request.args.get('time')
    if time: data = get_data(csv_file_path + "solar.csv", solar_schema, time=time)
    else: data = get_data(csv_file_path + "solar.csv", solar_schema, id=id)
    if not data: return APIResponse.error("Data not found", 404).make()
    return APIResponse.get_success(GetData(True, data)).make()
...
def post_chassis():
    ...
    if not id: return APIResponse.error("Failed to add data", 500).make()
    new_data_url = url_for('api.get_chassis_data_by_id', id=id)
    return APIResponse.post_success(AddData(True, id, 'chassis'), new_data_url).make()

@api.route("/telemetry/chassis/", methods=["GET"])
def get_chassis_data():
    data = get_data_all(csv_file_path + "chassis.csv", chassis_schema)
    if not data: return APIResponse.error("Failed to get data or not found", 500).make()
    return APIResponse.get_success(GetData(True, data)).make()
...
def get_chassis_data_by_id(id):
    time = request.args.get('time')
    if time: data = get_data(csv_file_path + "chassis.csv", chassis_schema, time=time)
    else: data = get_data(csv_file_path + "chassis.csv", chassis_schema, id=id)
    if not data: return APIResponse.error("Data not found", 404).make()
    return APIResponse.get_success(GetData(True, data)).make()
